...
function onOpenInTab(page) {
    
const showPage = document.getElementById('showPage');
    if (showPage) {
        showPage.innerHTML = '';
        showPage.innerHTML = `./pages/${page}`;
    } else {
        console.warn('Element with id "showPage" not found.');
    }

    //href="/pages/${page}" target="_blank"
}
    ...
    fetch('/list-pages')
        ...
        .then(data => {
            const listItems = data.map(page =>
                ...
                <a href="javascript:void(0)" onclick="onOpenInTab(${page})>${page}</a>
                    <div class="action">
